...
with reference_table as (
    {{ get_combination_calendar_dimensions_bymonth(min_date) }}
),

returning_users as (
    select
        *,
        date_trunc(date(date_day), month) as date_month
...
final as (
    ...
    left join returning_users
        on reference_table.date_month = returning_users.date_month
